local UI = {}
function UI.Create()
    local sg = Instance.new("ScreenGui", game.Players.LocalPlayer.PlayerGui); sg.Name = "Ble1zX_Hub"
    ...
    local topBar = Instance.new("TextLabel", main); topBar.Size = UDim2.new(1, 0, 0, 35); topBar.Text = "   Ble1zX Hub v5.0"; topBar.TextColor3 = Color3.new(1,1,1)
    ...
    local themes = {
        ...
        White = {bg = Color3.fromRGB(240, 240, 240), btn = Color3.fromRGB(200, 200, 200), txt = Color3.new(0,0,0), on = Color3.fromRGB(100, 200, 100), off = Color3.fromRGB(200, 100, 100)}
    }

    local allElements = {}
    local function applyTheme(k)
        _G.CT = k; local t = themes[k]; main.BackgroundColor3 = t.bg; topBar.TextColor3 = t.txt
    ...
    btn(farmP, "Field: ".._G.SelectedField, 50, function(b)
    ...
    btn(miscP, "Anti-AFK: ON", 220, function(b) _G.AntiAFK = not _G.AntiAFK; b:SetAttribute("Active", _G.AntiAFK); b.Text = _G.AntiAFK and "Anti-AFK: ON" or "Anti-AFK: OFF"; applyTheme(_G.CT) end, true):SetAttribute("Active", true)

    -- Контент Theme
    btn(themeP, "Dark Theme", 10, function() applyTheme("Dark") end)
    btn(themeP, "White Theme", 50, function() applyTheme("White") end)
    btn(themeP, "Atlas Theme", 90, function() applyTheme("Atlas") end)
